...
String pp="";
String p1=request.getParameter("c1");
if(p1!=null){
	b1=true;
	pp+=p1+" ";
}
String p2=request.getParameter("c2");
if(p2!=null){
	b2=true;
	pp+=p2+" ";
}
String p3=request.getParameter("c3");
if(p3!=null){
	b3=true;
	pp+=p3;
}
...
<header>
	...
	<map name="homemap"><area shape="rect" coords="0,0,682,682" href="main_after_login.jsp">
	</map>
	<div style="top:5px">
	<span><%=search%>의 <%=pp%>에 관한 검색 결과입니다. <br>다시 검색하려면 아이콘이나 검색 메뉴를 누르세요.</span>
	</div>
...
try {
	...
	String s1="title LIKE '%"+search+"%'";
	String s2="content LIKE '%"+search+"%'";
	String s3="user_name LIKE '%"+search+"%'";
	String ss="";
	if(b1){
		ss+=s1;
	}if(b2){
	...
	sql = "SELECT * FROM board INNER JOIN user USING(user_idx) WHERE "+ss+" ORDER BY board_idx DESC;";
