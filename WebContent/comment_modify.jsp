...
String c=String.valueOf(request.getParameter("c"));	//url 통해 수정할 comment_idx넘겨받음
int comment_idx=Integer.parseInt(c);

try {
	con = DriverManager.getConnection(DB_URL, DB_USER, DB_PASSWORD);

	String sql = "SELECT * FROM board INNER JOIN user USING(user_idx) WHERE board_idx=?;";
...
<article>
	...
	<div>
		...
		%>
		<img id="글사진" src="./upload/<%=rs2.getString("appendfile") %>" >
		<%
			}
		}
		rs2.close();
		...
		while(rs3.next()){
			...
			int modifyComment = rs3.getInt("comment_idx");
			if(modifyComment==comment_idx){	//수정 할 댓글 이면 입력창 띄움
			%>
			<script>
			window.onload=function(){
				document.getElementById("modify_comment").focus();
			}
			</script>
			
			<form id="수정댓글창" action="comment_modify_do.jsp?i=<%=rs3.getInt("board_idx")%>&c=<%=rs3.getInt("comment_idx")%>" method="post">
			<span>
			<input type="text" name="comment" id="modify_comment" size="100" maxlength="100" value="<%=rs3.getString("content")%>">
			<input type="submit" value="저장">
			</span>
			</form>
						
			<%
			}else{	//수정하는 댓글이 아니면 그냥 띄움
			%>
